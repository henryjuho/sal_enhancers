...
def polarisable(vcf_var, wga_bed):
    ...
    # salmon, char and trout first 3 columns
    var_align = [x.upper() for x in var_align[0:3]]
...
def main():
    ...
    vcf = VariantFile(all_sites)
    samples = list(vcf.header.samples)

    for line in vcf.fetch(chromosome, start, stop):

        str_line = str(line)
        # catch indels that overlap start and trim
        position = int(line.pos)
        if 'END' not in str_line and position < start + 1:
            continue

        # catch lines with END in
        block_len = 1

        if 'END' in str_line:
            end = int(str_line.split('END=')[1].split()[0])

            # if whole region in block - unlikely
            if position < start + 1 <= stop < end:
                block_len = stop - start
            
            # if start is mid block
            elif position < start + 1 <= end:
                block_len = end - start
            
            # if end is mid block
            elif position < stop < end:
                block_len = stop + 1 - position

            elif position == stop:
                block_len = 1

            else:
                block_len = (end - position) + 1
        ...
        # check for ns
        if line.ref == 'N':
            fasta_string += '0' * block_len
            continue

        # depth filter
        try:
            cumulative_depth = sum([float(line.samples[x]["DP"]) for x in samples if line.samples[x]["DP"] is not None])
        except KeyError:
            try:
                cumulative_depth = line.info["DP"]
            except KeyError:
                fasta_string += '1' * block_len
                continue
