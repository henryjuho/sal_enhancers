...
def main():
    ...
    parser.add_argument('-keep_indels', help='if specified retains indels', default=False, action='store_true')
    ...
    extracted_data = []
    out_fastas = []
    include_spp = args.spp.split(',')
    fastas_to_merge = [args.out_stem + '.' + spp_name + '.fa' for spp_name in include_spp]
    ...
    # cat cmd
    cat = 'cat {fastas} > {z}.fa'.format(fastas=' '.join(fastas_to_merge), z=args.out_stem)
