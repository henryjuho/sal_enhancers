...
def main():
    ...
    for line in gzip.open(fa):
        ...
        if line.startswith('>'):
            ...
            sequence = ''
            chromo = '_'.join(line.split('[')[0].split('|')[1].split('_')[0:2])
            trans_name = '_'.join(line.split('[')[0].split('|')[1].split('_')[2:])
            coord_data = line.split()[-2].rstrip(']')
            ...
            # catch imprecise coords
            if '>' in coord_data or '<' in coord_data:
                skip = True
                continue
            ...
            coord_string = coord_data.split('(')[-1].rstrip(')').split('location=')[-1]
