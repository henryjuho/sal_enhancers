...
def main():
    ...
    parser.add_argument('-vcf', help='VCF file containing variants to polarise', required=True)
    ...
    if args.sub is True:
        ...
        q_sub(command_line, out=args.vcf.replace('.vcf', 'polarisation'), scheduler='SLURM')
        sys.exit()

    # variables
    vcf_file = args.vcf
    out_vcf = vcf_file.replace('.vcf.gz', '.polarised.vcf')
